...
const steps = [
  { id:1, type:"intro", title:"يوسف يتسوّق! 🛍️", subtitle:"ساعد يوسف في حساب مشترياته",
    story:[{emoji:"👦",text:"يوسف وفّر مصروفه اليومي طوال العام"},{emoji:"🏦",text:"عند تكسير حصّالته وجد 520,500 مليم"},{emoji:"📺",text:"شاهد إعلاناً بمناسبة العيد"},{emoji:"🎁",text:"يريد مفاجأة أختيه وشراء مستلزمات العيد"}]},
  { id:2, type:"mcq", title:"كم يملك يوسف؟ 💰", question:"اختر المبلغ الصحيح الذي وجده يوسف في حصّالته",
    options:["250,500 مليم","520,500 مليم","502,500 مليم","525,000 مليم"], correct:1, explanation:"✅ صحيح! يوسف يملك 520,500 مليم"},
  { id:3, type:"prices", title:"أسعار المشتريات 🏷️", subtitle:"تعرّف على أسعار كل المنتجات",
    items:[{emoji:"👕",name:"كسوة العيد (للواحد)",price:"120,000 مليم",count:"× 3 (ليوسف وأختيه)"},{emoji:"🎮",name:"لعبة فيديو",price:"196,500 مليم",count:"× 1"},{emoji:"🧸",name:"دبي محشو (للواحدة)",price:"35,600 مليم",count:"× 2 (لأختيه)"}]},
  { id:4, type:"mcq", title:"احسب ثمن الكسوات 👕", question:"يريد يوسف شراء كسوة له ولأختيه (3 أشخاص). كم يكلّف ذلك؟",
    hint:"120,000 × 3 = ؟", options:["240,000 مليم","360,000 مليم","300,000 مليم","480,000 مليم"], correct:1, explanation:"✅ رائع! 120,000 × 3 = 360,000 مليم",
    visual:["👕 120,000","👕 120,000","👕 120,000"]},
  { id:5, type:"mcq", title:"احسب ثمن الدببة 🧸", question:"يريد يوسف شراء دبّيْن محشوّيْن لأختيه. كم يكلّف ذلك؟",
    hint:"35,600 × 2 = ؟", options:["71,200 مليم","71,600 مليم","35,600 مليم","70,200 مليم"], correct:0, explanation:"✅ ممتاز! 35,600 × 2 = 71,200 مليم",
    visual:["🧸 35,600","🧸 35,600"]},
  { id:6, type:"sum", title:"المجموع الكلّي 🧮", question:"احسب مجموع كل المشتريات",
    rows:[{emoji:"👕",label:"كسوات العيد (×3)",value:360000},{emoji:"🎮",label:"لعبة فيديو",value:196500},{emoji:"🧸",label:"دبّة محشوة (×2)",value:71200}],
    options:["617,700 مليم","627,700 مليم","637,700 مليم","607,700 مليم"], correct:1, explanation:"✅ 360,000 + 196,500 + 71,200 = 627,700 مليم"},
  { id:7, type:"compare", title:"هل يكفيه المبلغ؟ ⚖️", question:"قارن بين ما يملكه يوسف وما يحتاجه",
    budget:520500, total:627700, options:["نعم، يكفيه المبلغ","لا، لا يكفيه المبلغ"], correct:1,
...
function CompareStep({ step, onNext, onWrong }) {
  ...
  return (
    <div>
      ...
      {done && (
        <div className={`feedback ${ok ? "fb-correct" : "fb-wrong"}`}>
          {step.explanation}
...
export default function App() {
  ...
  const confettiColors = ["#FF4B4B","#58CC02","#1CB0F6","#FFC800","#CE82FF","#FF9600"];
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .intro-icon { font-size: 56px; margin-bottom: 6px; }
        .intro-title { font-size: 22px; font-weight: 900; color: #3C3C3C; margin-bottom: 4px; }
        .intro-sub { color: #AFAFAF; font-size: 13px; margin-bottom: 18px; }

        .story-list { display: flex; flex-direction: column; gap: 10px; margin-bottom: 22px; }
        ...
        /* Prices */
        .price-list { display: flex; flex-direction: column; gap: 10px; margin-bottom: 20px; }
        ...
        .option-btn:hover:not(.opt-correct):not(.opt-wrong):not(.opt-selected) {
          border-color: #1CB0F6;
          background: #EDF8FF;
        }
        ...
        .trophy { font-size: 64px; animation: bounce 0.6s ease infinite alternate; display: block; margin-bottom: 4px; }
        ...
        .summary-box {
          background: linear-gradient(135deg, #D7FFB8, #B8F0FF);
          border-radius: 12px;
          padding: 12px 14px;
        ...
        <div className="card">
          ...
          {step.type === "prices" && (
            <div>
              ...
              <button className="main-btn btn-blue" onClick={next}>فهمت! →</button>
            </div>
          )}
          ...
          {step.type === "mcq" && (
            <div>
              <h2 className="step-title">{step.title}</h2>
              <MCQ key={step.id} step={step} onNext={next} onWrong={wrong} />
            </div>
          )}
          ...
          {step.type === "sum" && (
            <div>
              <h2 className="step-title">{step.title}</h2>
              <SumStep key={step.id} step={step} onNext={next} onWrong={wrong} />
            </div>
          )}
          ...
          {step.type === "compare" && (
            <div>
              <h2 className="step-title">{step.title}</h2>
              <CompareStep key={step.id} step={step} onNext={next} onWrong={wrong} />
            </div>
          )}
          ...
          {step.type === "finish" && (
            <div className="finish-center">
              ...
              <div className="summary-box">
                ...
                👕 كسوات (×3): 360,000 م<br/>
                🎮 لعبة فيديو: 196,500 م<br/>
                🧸 دبب (×2): 71,200 م<br/>
                ───────────────<br/>
                🛒 المجموع: 627,700 م<br/>
                💰 عنده: 520,500 م<br/>
                ❌ ينقصه: 107,200 م
              </div>
            </div>
